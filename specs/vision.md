...
5.**Criação do relatório** de uso do espaço, com análise de usuários e frequência por curso.
...
| **Comunicação Eficiente e Clara** | Falta de notificações automáticas sobre o status dos agendamentos (aprovado e negado). | Solicitantes ( alunos) |
| **Transparência e Centralização** | Desorganização e "confusão" causadas por informações descentralizadas (Google Agenda e WhatsApp). Professores bloqueiam espaços que acabam não utilizando. | Todos os usuários (Alunos, DCE e CEF) |
| **Responsabilização e Auditoria** | Falta de registro formal de qual indivíduo utilizou o espaço. A responsabilidade por danos recai inteiramente sobre o DCE, que assina termos físicos. |Alunos, DCE e CEF |
| **Acesso Justo (Democratização)** | A regra atual de "quem pediu primeiro" no WhatsApp não garante a hierarquia oficial de prioridades (Aulas > Projetos > Alunos), limitando a "democratização" do espaço. | Alunos, DCE e CEF) |
...
| :--- | :--- | :--- |
| **DCE (Diretório Central dos Estudantes)** | Entidade que representa os estudantes. Gerencia o agendamento dos horários livres disponíveis para os membros da comunidade acadêmica. | Usuário (Moderador/Gestor) |
| **CEF (Coordenação de Educação Física)** | Órgão responsável pela gestão principal dos espaços esportivos e gerenciamento do calendário de aulas. | Cliente, Usuário (Moderador/Gestor) |
| **Alunos** | Os alunos precisam consultar a disponibilidade ou solicitar o uso dos espaços. | Usuários (Solicitante, Visualizador) |
...
2.  **Solicitantes (Alunos)** precisam se autenticar (via SUAP) para submeter pedidos de agendamento.
...
| **Segurança:** O sistema deve garantir que apenas usuários autorizados possam pedir horários e que apenas o moderador DCE pode aprovar/negar alterações e cancelamentos. | **Alta** | A segurança garante a legitimidade dos pedidos e a integridade do calendário. | Inexistente. O controle de acesso no WhatsApp é frágil e a edição do Google Agenda é centralizada em uma pessoa. | Integração com SUAP para autenticação. Implementar Controle de Acesso para os diferentes perfis (Solicitante(Aluno), Moderador (DCE e CEF)). |
...
Além disso, o sistema SIGAE será fundamentalmente integrado com o SUAP. Essa integração ocorrerá por meio de APIs e será utilizada para a autenticação dos usuários e para a importação de dados que foram cadastrados.
...
* **Sustentabilidade (Manutenção):** A manutenção pós-projeto será de responsabilidade da instituição. A **DTI do IFPB** manterá os servidores no ar, enquanto a **equipe de desenvolvimento** (futuros bolsistas designados) ficará responsável por correções de bugs e melhorias futuras.
...
2. O sistema deve permitir que **Solicitantes** (Alunos) submetem solicitações de agendamento  para os horários disponíveis. 
...
4. O sistema deve possuir um **painel de solicitação**  onde os Moderadores o DCE possam aprovar ou negar as solicitações de alteração e cancelamento.
5. O sistema deve **enviar notificações automáticas** preferencialmente por e-mail, aos solicitantes informando o status do cancelamento ou alteração (aprovado, negado).
...
10. O sistema deve permitir que **Moderadores/Gestores** visualizem um **dashboard com estatísticas** de uso dos espaços.
